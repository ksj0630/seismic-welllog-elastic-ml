...
fin1 = open('../../01.seismic_velocity_equal_size/coord_vel_wind_cdp_cdpt.bin.resize_4500',"rb")
...
for ii in range(1,nwell):
    ...
    extrt[kk,2] = seis[loc,iz] # 해당 위치에서의 seismic velocity value
    kk = kk+1


extrs = np.float32(extrs)
fout = open("../01.well_data/05.extract_vel_4.bin","wb")
extrs.tofile(fout)

extrt[:,2] = extrt[:,2]*0.001 # seismic velocity 단위(m/s) -> 0.001 곱해서 km/s 단위 변환
np.savetxt("../01.well_data/05.extract_veltrace_4.txt",extrt,fmt='%f')
