...
# Config
# -----------------------------
SAVE_DIR   = Path("../06.model_save_folder/38.code/")
OUTPUT_DIR = Path("../05.result/38.code/")
SAVE_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TRAIN_PATH = Path("../36.LSTM_GP_merge_data_for_DTS/01.X_train_1A_11T2_14.txt")
APPLY_PATH = Path("../35.LSTM_GP_X_train_for_DTS/02.4_X_train_index+10.txt")
AUX_PATH   = Path("../34.LSTM_GP_train_data_for_DTS/02.4_for_dts.txt")  # kept for consistency
...
def load_data():
    ...
    _aux = np.loadtxt(AUX_PATH)  # not directly used in pipeline
    return (x_train, y_train), (r_train, z_train)
...
def plot_test(depth, y_true, y_pred, y_std, out_png: Path):
    ...
    plt.plot(depth[:-8], y_true[:-8], label="True", linewidth=1.0, linestyle="solid", color="black")
    plt.plot(depth[:-8], y_pred[:-8], label="Pred", linewidth=1.0, linestyle="solid", color="red")
...
def main(epochs=EPOCHS, batch_size=BATCH_SIZE, gpiter=GP_RESTARTS):
    (x_train, y_train), (r_train, z_train) = load_data()
    ...
    # feature vector: penultimate Dense (i.e., layers[-2]) → shape (None, 16)
    ...
    plot_train(Y_train, y_pred_tr, y_std_tr, OUTPUT_DIR / "38.merge_pred.png")

    mid = X_test.shape[1] // 2
    depth = X_test[:, mid, 0]
    pack = np.column_stack([depth, Y_test, y_pred_te])
    np.savetxt(OUTPUT_DIR / "38.pred_dtc.txt", pack, delimiter=" ")
    ...
    plot_test(depth, Y_test, y_pred_te, y_std_te, OUTPUT_DIR / "38.pred.png")
