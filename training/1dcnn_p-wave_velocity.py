...
def main(n_ch=3, epochs=1000, batch_size=128, structure="CNN1D"):
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "87.merge_pred.png")); plt.close()

    np.savetxt(os.path.join(OUTPUT_DIR, "22.F4_true.txt"), data.y_apply)
    np.savetxt(os.path.join(OUTPUT_DIR, "23.F4_pred.txt"), y_pred2)

    plt.figure(figsize=(20, 2))
    plt.plot(tvd3[10:-10], data.y_apply, label="True", linewidth=1.0, color="black")
    plt.plot(tvd3[10:-10], y_pred2,      label="Pred", linewidth=1.0, color="red")
    plt.plot(tvd3[10:-10], init3[10:-10], label="initial", linewidth=1.0, linestyle="dashed", color="blue")
    plt.legend(); plt.xlim([tvd3[10], tvd3[-10]]); plt.ylim([1, 5.75])
    plt.xlabel("Depth (km)"); plt.ylabel("P-wave velocity (km/s)"); plt.grid(linestyle=":")
    plt.savefig(os.path.join(OUTPUT_DIR, "87.pred.png")); plt.close()

    half  = data.X_apply.shape[1] // 2
    depth = data.X_apply[:, half, 0]
    pred_pack = np.column_stack([depth, data.y_apply, y_pred2.squeeze()])
    np.savetxt(os.path.join(OUTPUT_DIR, "87.pred_dtc.txt"), pred_pack, delimiter=" ")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="1D CNN for P-wave velocity")
