...
# Load_Data
###########################
fin1 = open("../16.LSTM_merge_data_from_15.folder/00.X_train_1A_5_11T2_14_15A_for_density.txt","r")
...
fin2 = open("../15.LSTM_X_train_for_density/02.4_X_train_index+10.txt","r")
...
def show_MultiHeadAttention_LSTM_model(SINGLE):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '101.merge_code.png'))
    # plt.show()
    ...
    plt.figure(figsize=(20, 2))
    # DTC 예측할 때, 이미 -20되었음, 그래서 tvd3가 [20:-20]인 것
    plt.plot(tvd3[20:-20], Z_train[:], label="True", linewidth=1.0, linestyle='solid', color="black")
    plt.plot(tvd3[20:-20], predict2[:], label="Pred", linewidth=1.0, linestyle='solid', color="red")
    plt.legend()
    ...
    plt.ylabel("Density (g/cm3)")
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '101.code.png'))
    # plt.show()

#################################
# Load_the_last_model
#################################
def main(epochs=529, batch_size=128):
