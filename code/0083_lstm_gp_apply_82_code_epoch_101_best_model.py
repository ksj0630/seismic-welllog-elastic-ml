...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/83.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/83.code/")
...
# Load_Data
###########################
fin1 = open("../15.LSTM_GP_merge_data_for_DTS/01.X_train_1A_11T2_14.txt","r")
...
fin2 = open("../14.LSTM_GP_X_train_for_DTS/02.4_X_train_index+10.txt","r")
...
fin3 = open('../13.LSTM_GP_train_data_for_DTS/02.4_for_dts.txt',"r")
...
# Load_the_last_model
#################################
def main(epochs=101, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '83.pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "83.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    # ✅ 눈금 크기 키우기
    ytick_values = np.arange(1.0, 3.1, 0.5)  # np.arange(시작, 끝+0.01, 간격)
    ...
    # plt.savefig(os.path.join(output_dir, '502.pred.png'))
    # plt.show()

    # 저장할 때 여백 없애기
    plt.savefig(os.path.join(output_dir, '83.pred_fontsize_20_dpi300.png'), bbox_inches='tight', pad_inches=0.05, dpi=300)
