...
class save_last(Callback):
    def on_epoch_end(self, epoch, logs=None):
        if epoch == self.params['epochs'] - 1:
            self.model.save(os.path.join('../06.model_save_folder/43.code/last_epoch_model.hdf5'))

def main(n_ch=3, epochs=150, batch_size=128, gpiter=20, structure='CNN1D'):

    data_file = ['../29.1DCNN_GP_train_data_for_density/x_train_rhob.npy', '../29.1DCNN_GP_train_data_for_density/y_train_rhob.npy']
    apply_file = ['../29.1DCNN_GP_train_data_for_density/x_test_rhob.npy', '../29.1DCNN_GP_train_data_for_density/y_test_rhob.npy']
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '43.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:,2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "43.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.figure(figsize=(20, 2))
    plt.plot(depth, data.y_apply, label='True', linewidth=1.0, linestyle='solid', color="black")
    ...
    plt.grid(linestyle=':')
    # plt.show()
    plt.savefig(os.path.join(output_dir, '43.pred.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-epochs', type=int, default=198, help='training epochs (default: 300)')
