...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/86.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/86.code/")
...
def main(X_train, X_test, Y_train, Y_test, save_dir, output_dir, epochs=1000, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '86.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "86.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.xlim(depth[10], depth[-18])
    plt.ylim([1, 5.75])
    plt.xticks(fontsize=20)
    # ✅ 눈금 크기 키우기
    ytick_values = np.arange(1.0, 5.9, 1.0)
    plt.yticks(ytick_values, fontsize=20)
    # ✅ y축 눈금을 항상 소수점 1자리까지 표시
    ...
    # plt.savefig(os.path.join(output_dir, '10.pred.png'))
    # plt.show()

    # 저장할 때 여백 없애기
    plt.savefig(os.path.join(output_dir, '86.pred_fontsize_20_dpi300.png'), bbox_inches='tight', pad_inches=0.05, dpi=300)
...
if __name__ == '__main__':
    ...
    for feature in range(3):  # 3개의 특성이 있다고 가정
        ...
        output_dir = os.path.join(f"../../12.SCI_Discussion/05.result/86.code/shuffle_{int(shuffle_ratio*100)}_feature_{feature}/")
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # 모델 저장 디렉토리 설정 (특성별로 다르게)
        save_dir = os.path.join(f"../06.model_save_folder/86.code/shuffle_{int(shuffle_ratio*100)}_feature_{feature}/")
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
