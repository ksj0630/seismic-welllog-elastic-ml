...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/28.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/28.code/")
...
def main(epochs=1000, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '28.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "28.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.grid(linestyle=':')
    # plt.show()
    plt.savefig(os.path.join(output_dir, '28.pred.png'))
