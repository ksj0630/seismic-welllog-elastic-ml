...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/85.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/85.code/")
...
def main(X_train, X_test, Y_train, Y_test, save_dir, output_dir, epochs=1000, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '85.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "85.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    np.savetxt(os.path.join(output_dir, "24.F4_initial.txt"), init3)
    ...
    plt.plot(depth[:], init3[10:-10], label='initial', linewidth=1.0, linestyle='dashed', color="blue")
    ...
    plt.ylabel("P-wave velocity (km/s)")
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '85.pred_2.png'))
    # plt.show()
...
if __name__ == '__main__':
    ...
    # 특정 특성(feature)을 0으로 설정하는 함수
    def zero_out_feature(data, feature_idx):
        data_mod = data.copy()
        data_mod[:, :, feature_idx] = 0  # 해당 특성(feature)을 0으로 설정
        return data_mod

    # OAT 실험: 3개의 특성(feature) 중 하나를 0으로 만들어 학습
    for feature in range(3):  # 3개의 특성이 있다고 가정
        print(f"\n=== Feature {feature}을(를) 0으로 설정하고 학습 시작 ===")

        # OAT 적용 데이터셋 생성 (각 특성별로 번갈아가면서 0으로 설정)
        X_train_oat = zero_out_feature(X_train, feature)
        X_test_oat = zero_out_feature(X_test, feature)

        # 실험 결과를 저장할 디렉토리 설정 (특성별로 다르게)
        output_dir = os.path.join(f"../05.result/85.code/feature_{feature}/")
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # 모델 저장 디렉토리 설정 (특성별로 다르게)
        save_dir = os.path.join(f"../06.model_save_folder/85.code/feature_{feature}/")
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        # 모델 학습 실행 (특성별 0처리된 데이터로 학습)
        main(X_train_oat, X_test_oat, Y_train, Y_test, save_dir, output_dir)
