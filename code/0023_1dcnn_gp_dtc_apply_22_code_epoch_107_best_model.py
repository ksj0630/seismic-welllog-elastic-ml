...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/23.code/') 
if not os.path.exists(save_dir):
    os.mkdir(save_dir)

output_dir = os.path.join("../05.result/23.code/")
...
class DATA_read():
    def __init__(self, data_file, apply_file):
        ...
        # 학습용, 테스트용 데이터 분할
        X_train, X_test, y_train, y_test = train_test_split(X_scaled, y_half, test_size=0.1, random_state=42, shuffle=False)
...
class save_last(Callback):
    def on_epoch_end(self, epoch, logs=None):
        if epoch == self.params['epochs'] - 1:
            self.model.save(os.path.join('../06.model_save_folder/23.code/last_epoch_model.hdf5'))

def main(n_ch=3, epochs=50, batch_size=128, gpiter=20, structure='CNN1D'):

    data_file = ['../13.1DCNN_input_from_prof/training/x_train.npy', '../13.1DCNN_input_from_prof/training/y_train.npy']
    apply_file = ['../13.1DCNN_input_from_prof/training/x_test.npy', '../13.1DCNN_input_from_prof/training/y_test.npy']
    ...
    # LSTM 모델로 특징 벡터 추출
    features_train = feature_extractor.predict(data.X_train)
    features_test = feature_extractor.predict(data.X_test)
    features_test2 = feature_extractor.predict(data.X_apply)
    ...
    if initial_epoch == epochs and os.path.isfile(gpfile):
        ...
        gp.fit(features_train, data.y_train)

    # Gaussian Process 모델 저장
    joblib.dump(gp, gpfile)

    # GP로 예측 및 불확실성 추정
    y_pred, y_std = gp.predict(features_test, return_std=True)
    y_pred2, y_std2 = gp.predict(features_test2, return_std=True)
    ...
    print(f"y_pred shape: {y_pred.shape}, y_std shape: {y_std.shape}")  
    ...
    plt.fill_between(y_pred - 2*y_std, y_pred + 2*y_std, color='gray', alpha=0.2, label='Uncertainty')
    plt.legend()
    # plt.xlim(2.556, 3.051)
    plt.xlabel("Depth (km)")
    plt.ylabel("P-wave velocity (km/s)")
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '23.merge_pred.png'))
    # plt.show()
    ...
    plt.figure(figsize=(20, 2))
    plt.plot(tvd3[10:-10], data.y_apply, label='True', linewidth=1.0, linestyle='solid', color="black")
    plt.plot(tvd3[10:-10], y_pred2, label='Pred', linewidth=1.0, linestyle='solid', color="red")
    plt.plot(tvd3[10:-10], init3[10:-10], label='initial', linewidth=1.0, linestyle='dashed', color="blue")
    plt.fill_between(tvd3[10:-10], y_pred2 - 2*y_std2, y_pred2 + 2*y_std2, color='gray', alpha=0.2, label='Uncertainty')
    plt.legend()
    ...
    plt.grid(linestyle=':')
    # plt.show()
    plt.savefig(os.path.join(output_dir, '23.pred.png'))
    ...
    pred_dtc[:,2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "23.pred_dtc.txt"), pred_dtc, delimiter=' ')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-epochs', type=int, default=107, help='training epochs (default: 300)')
    parser.add_argument('-gpiter', type=int, default=20, help='gaussian process iteration (default: 10)')
    parser.add_argument('-batch_size', type=int, default=128, help='batch size (default: 64)')
    parser.add_argument('-model', type=str, default='CNN1D', help='model name to save (default: UNET)')
