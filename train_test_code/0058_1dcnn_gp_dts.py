...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/58.code/') 
if not os.path.exists(save_dir):
    os.mkdir(save_dir)

output_dir = os.path.join("../05.result/58.code/")
...
class save_last(Callback):
    def on_epoch_end(self, epoch, logs=None):
        if epoch == self.params['epochs'] - 1:
            self.model.save(os.path.join('../06.model_save_folder/58.code/last_epoch_model.hdf5'))

def main(n_ch=3, epochs=150, batch_size=128, gpiter=20, structure='CNN1D'):

    data_file = ['../30.1DCNN_GP_train_data_for_dts/x_train_dts.npy', '../30.1DCNN_GP_train_data_for_dts/y_train_dts.npy']
    apply_file = ['../30.1DCNN_GP_train_data_for_dts/x_test_dts.npy', '../30.1DCNN_GP_train_data_for_dts/y_test_dts.npy']
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '58.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:,2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "58.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.figure(figsize=(20, 2))
    plt.plot(depth[0:-8], data.y_apply[0:-8], label='True', linewidth=1.0, linestyle='solid', color="black")
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '58.pred.png'))
    # plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-epochs', type=int, default=50, help='training epochs (default: 300)')
