...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/174.code/') 
if not os.path.exists(save_dir):
    os.mkdir(save_dir)

output_dir = os.path.join("../05.result/174.code/")
...
def main(n_ch=3, epochs=1000, batch_size=128, structure='CNN1D'):
    ...
    data_file = ['../18.1DCNN_train_data_for_dts/x_train_dts.npy', '../18.1DCNN_train_data_for_dts/y_train_dts.npy']
    apply_file = ['../18.1DCNN_train_data_for_dts/x_test_dts.npy', '../18.1DCNN_train_data_for_dts/y_test_dts.npy']
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '174.merge_pred.png'))
    # plt.show()
    ...
    np.savetxt(os.path.join(output_dir, "174.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.legend()
    plt.xlim([depth[0], depth[-8]]) # 마지막에 결측값 구간있어서 8개 추가로빼버림 
    plt.ylim([1.2, 2.9])
    plt.xlabel("Depth (km)")
    plt.ylabel("S-wave velocity (km/s)")
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '174.pred.png'))
    # plt.show()
