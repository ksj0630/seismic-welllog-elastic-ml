...
fin3 = open('../01.GBT_whole_data/12.new_process_1A_GBT_whole.txt',"r")
...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/11.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/11.code/")
...
fin2 = open("../02.X_train/01.1A_X_train_index+10.txt","r")
...
def main(epochs=800, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '11.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "11.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.grid(linestyle=':')
    # plt.show()
    plt.savefig(os.path.join(output_dir, '11.pred.png'))
