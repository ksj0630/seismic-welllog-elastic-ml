...
# save_model
#################################
save_dir = os.path.join('../06.model_save_folder/71.code/')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

output_dir = os.path.join("../05.result/71.code/")
...
# Load_Data
###########################
fin1 = open("../12.LSTM_GP_merge_data_for_density/01.X_train_1A_5_11T2_14_15A.txt","r")
...
fin2 = open("../11.LSTM_GP_X_train_for_density/02.4_X_train_index+10.txt","r")
...
# Load_the_last_model
#################################
def main(epochs=961, batch_size=128, gpiter=20):
    ...
    plt.grid(linestyle=':')
    plt.savefig(os.path.join(output_dir, '71.merge_pred.png'))
    # plt.show()
    ...
    pred_dtc[:, 1] = Z_train[:]
    pred_dtc[:, 2] = y_pred2[:]
    np.savetxt(os.path.join(output_dir, "71.pred_dtc.txt"), pred_dtc, delimiter=' ')
    ...
    plt.grid(linestyle=':')
    # plt.show()
    plt.savefig(os.path.join(output_dir, '71.pred.png'))
    ...
    # 저장할 때 여백 없애기
    plt.savefig(os.path.join(output_dir, '71.pred_fontsize_20_dpi300_same_depth_raw_rhob.png'), bbox_inches='tight', pad_inches=0.05, dpi=300)
